...
function SolutionsPage(props: SolutionsPageProps) {
    ...
    return router
      .push({
        ...
        query: api.serializeParam({
          ...
          size: pageable.size,
          sort:
            pageable.sort === Pageable.Sorts.UNSORT ? undefined : pageable.sort,
  ...
  useEffect(() => {
    if (props.data?.length > 0)
      setSelected(Array(props.data?.length).fill(false));
  }, [props.data]);
          ...
          <div className="solution-sider">
            ...
            >
              <Card
                ...
                title={
                  "Detail " + (detail.data?.name ? detail.data.name : "Kendala")
                }
                hoverable
                extra={
                  <Space>
...
export async function getServerSideProps(ctx: NextPageContext) {
  ...
  const total = Number(
    res.headers[HttpHeader.X_TOTAL_COUNT] || res.data.length
  );
  return {
    props: {
      total,
